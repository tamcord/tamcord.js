...
class ApplicationCommandPermissionsManager extends BaseManager {
    constructor(manager) {
         ...
         * @type {?Guild}
         */
        this.guild = (_a = manager.guild) !== null && _a !== void 0 ? _a : null;
        /**
         * The id of the guild that this manager acts on
         * @type {?Snowflake}
         */
        this.guildId = (_d = (_b = manager.guildId) !== null && _b !== void 0 ? _b : (_c = manager.guild) === null || _c === void 0 ? void 0 : _c.id) !== null && _d !== void 0 ? _d : null;
        /**
         * The id of the command this manager acts on
         * @type {?Snowflake}
         */
        this.commandId = (_e = manager.id) !== null && _e !== void 0 ? _e : null;
    }
    /**
    ...
    fetch({ guild, command } = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (commandId) {
                const data = yield this.permissionsPath(guildId, commandId).get();
                return data.permissions.map(perm => this.constructor.transformPermissions(perm, true));
            }
            const data = yield this.permissionsPath(guildId).get();
     ...
     *   .catch(console.error);
     */
    set({ guild, command, permissions, fullPermissions } = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            const { guildId, commandId } = this._validateOptions(guild, command);
            if (commandId) {
                ...
                const data = yield this.permissionsPath(guildId, commandId).put({
            ...
            for (const perm of fullPermissions) {
                if (!Array.isArray(perm.permissions))
                    throw new TypeError('INVALID_ELEMENT', 'Array', 'fullPermissions', perm);
                APIPermissions.push({
                    id: perm.id,
                    permissions: perm.permissions.map(p => this.constructor.transformPermissions(p)),
                });
            }
            const data = yield this.permissionsPath(guildId).put({
                data: APIPermissions,
            });
            return data.reduce((coll, perm) => coll.set(perm.id, perm.permissions.map(p => this.constructor.transformPermissions(p, true))), new Collection());
        });
    ...
    add({ guild, command, permissions }) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let existing = [];
            try {
                existing = yield this.fetch({ guild: guildId, command: commandId });
            }
            catch (error) {
    ...
    remove({ guild, command, users, roles }) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!commandId)
                throw new TypeError('INVALID_TYPE', 'command', 'ApplicationCommandResolvable');
            if (!users && !roles)
                throw new TypeError('INVALID_TYPE', 'users OR roles', 'Array or Resolvable', true);
            let resolvedIds = [];
            ...
            if (Array.isArray(roles)) {
                roles.forEach(role => {
                    ...
                    if (!this.guild)
                        throw new Error('GUILD_UNCACHED_ROLE_RESOLVE');
    ...
    _validateOptions(guild, command) {
        var _a, _b, _c, _d;
        const guildId = (_a = this.guildId) !== null && _a !== void 0 ? _a : this.client.guilds.resolveId(guild);
        if (!guildId)
            throw new Error('GLOBAL_COMMAND_PERMISSIONS');
        let commandId = this.commandId;
        if (command && !commandId) {
            commandId = (_c = (_b = this.manager).resolveId) === null || _c === void 0 ? void 0 : _c.call(_b, command);
            if (!commandId && this.guild) {
                commandId = this.guild.commands.resolveId(command);
            }
            if (!commandId) {
                commandId = (_d = this.client.application) === null || _d === void 0 ? void 0 : _d.commands.resolveId(command);
            }
            if (!commandId) {

...
class TextBasedChannel {
    ...
    send(options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const d = yield this.client.api.channels[this.id].messages.post({ data, files });
    ...
    sendTyping() {
        return __awaiter(this, void 0, void 0, function* () {
            yield this.client.api.channels(this.id).typing.post();
        });
    }
    /**
    ...
    createMessageComponentCollector(options = {}) {
        return new InteractionCollector(this.client, Object.assign(Object.assign({}, options), { interactionType: InteractionTypes.MESSAGE_COMPONENT, channel: this }));
    }
    /**
    ...
    bulkDelete(messages, filterOld = false) {
        return __awaiter(this, void 0, void 0, function* () {
            if (Array.isArray(messages) || messages instanceof Collection) {
                let messageIds = messages instanceof Collection ? [...messages.keys()] : messages.map(m => { var _a; return (_a = m.id) !== null && _a !== void 0 ? _a : m; });
                if (filterOld) {
                ...
                if (messageIds.length === 1) {
                    yield this.client.api.channels(this.id).messages(messageIds[0]).delete();
                    const message = this.client.actions.MessageDelete.getMessage({
                        message_id: messageIds[0],
                    }, this);
                    return message ? new Collection([[message.id, message]]) : new Collection();
                }
                yield this.client.api.channels[this.id].messages['bulk-delete'].post({ data: { messages: messageIds } });
                return messageIds.reduce((col, id) => col.set(id, this.client.actions.MessageDeleteBulk.getMessage({
                    message_id: id,
                }, this)), new Collection());
            }
            if (!isNaN(messages)) {
                const msgs = yield this.messages.fetch({ limit: messages });

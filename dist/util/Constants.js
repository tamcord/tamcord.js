...
const AllowedImageSizes = Array.from({ length: 9 }, (e, i) => Math.pow(2, (i + 4)));
...
exports.Endpoints = {
    CDN(root) {
        return {
            ...
            AppIcon: (appId, hash, { format = 'webp', size } = {}) => makeImageUrl(`${root}/app-icons/${appId}/${hash}`, { size, format }),
            AppAsset: (appId, hash, { format = 'webp', size } = {}) => makeImageUrl(`${root}/app-assets/${appId}/${hash}`, { size, format }),
            StickerPackBanner: (bannerId, format = 'webp', size) => makeImageUrl(`${root}/app-assets/710982414301790216/store/${bannerId}`, { size, format }),
